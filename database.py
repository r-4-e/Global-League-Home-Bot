...
class Database:
    ...
    # ── QOTD ─────────────────────────────────────────────────────────────────
    async def ensure_qotd_config(self) -> None:
        """Create the default config row if it doesn't exist yet."""
        try:
            existing = (
                await self.client.table("qotd_config")
                .select("id")
                .eq("guild_id", GUILD_ID)
                .maybe_single()
                .execute()
            ...
            log.error("ensure_qotd_config: %s", exc)

    async def get_qotd_config(self) -> Optional[dict]:
        try:
            res = (
                await self.client.table("qotd_config")
                .select("*")
            ...
            log.error("get_qotd_config: %s", exc)
            return None

    async def update_qotd_config(self, data: dict) -> bool:
        try:
            await (
                self.client.table("qotd_config")
                .update(data)
            ...
            log.error("update_qotd_config: %s", exc)
            return False

    async def set_qotd_last_posted(self, date_str: str) -> None:
        await self.update_qotd_config({"last_posted_date": date_str})

    async def increment_qotd_day_counter(self) -> int:
        try:
            cfg = await self.get_qotd_config()
            current = (cfg or {}).get("day_counter", 0)
            new_val = current + 1
            await self.update_qotd_config({"day_counter": new_val})
            return new_val
        except Exception as exc:
            log.error("increment_qotd_day_counter: %s", exc)
            return 1

    async def add_qotd_question(self, question: str, added_by: int) -> Optional[int]:
        try:
            res = (
                await self.client.table("qotd_questions")
                .insert({"guild_id": GUILD_ID, "question": question, "added_by": added_by})
                .execute()
            )
            if res.data:
                return res.data[0]["id"]
        except Exception as exc:
            log.error("add_qotd_question: %s", exc)
        return None

    async def remove_qotd_question(self, question_id: int) -> bool:
        try:
            res = (
                await self.client.table("qotd_questions")
                .delete()
            ...
            log.error("remove_qotd_question: %s", exc)
            return False

    async def get_next_qotd_question(self) -> Optional[dict]:
        """Returns the oldest unused question in the bank."""
        try:
            res = (
                await self.client.table("qotd_questions")
                .select("*")
            ...
            log.error("get_next_qotd_question: %s", exc)
            return None

    async def mark_qotd_used(self, question_id: int) -> None:
        try:
            await (
                self.client.table("qotd_questions")
                .update({"used": True})
                .eq("id", question_id)
                .execute()
            )
        except Exception as exc:
            log.error("mark_qotd_used: %s", exc)

    async def list_qotd_questions(self) -> list[dict]:
        try:
            res = (
                await self.client.table("qotd_questions")
                .select("*")
            ...
            log.error("list_qotd_questions: %s", exc)
            return []

    async def count_qotd_questions(self) -> int:
        try:
            res = (
                await self.client.table("qotd_questions")
            ...
            log.error("count_qotd_questions: %s", exc)
            return 0
