...
class EconomyCog(commands.Cog, name="Economy"):
    ...
    async def work(self, ctx: commands.Context) -> None:
        ...
        extra = await self._maybe_award(ctx, data["wallet"] + earned, data["bank"])
        await ctx.send(
    ...
    async def rob(self, ctx: commands.Context, user: discord.Member) -> None:
        ...
        if random.random() < fail_chance:
            ...
            extra = await self._maybe_award(ctx, robber["wallet"] + stolen, robber["bank"])
            await ctx.send(
    ...
    async def blackjack(self, ctx: commands.Context, bet: int) -> None:
        ...
        view = BlackjackView(
            cog=self, interaction=ctx, bet=bet,
...
class EconomyExtraCog(commands.Cog, name="EconomyExtra"):
    ...
    async def _maybe_award(self, ctx, new_wallet, bank) -> str:
        if not isinstance(ctx.author, discord.Member):
            return ""
    ...
    async def fish(self, ctx: commands.Context) -> None:
        ...
        extra = await self._maybe_award(ctx, nw, data["bank"])
    ...
    async def coinflip(self, ctx: commands.Context, bet: int, choice: str) -> None:
        ...
        extra = await self._maybe_award(ctx, nw, data["bank"]) if win else ""
    ...
    async def on_command_error(self, ctx: commands.Context, error: commands.CommandError) -> None:
        ...
        if isinstance(error, commands.MissingRequiredArgument):
            ...
            await ctx.send(f"❌ Invalid argument. Use `gl.help {ctx.command.name}` for correct usage.")
            ...
            import logging
            logging.getLogger(__name__).error("Economy error in %s: %s", ctx.command, error, exc_info=error)
            await ctx.send(f"❌ Something went wrong: `{type(error).__name__}: {str(error)[:100]}`")
