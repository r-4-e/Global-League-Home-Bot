...
from utils.permissions import gate_hierarchy, gate_permission, gate_warn, BOT_OWNER_ID
...
class ModerationCog(commands.Cog, name="Moderation"):
    ...
    async def warn(self, ctx, user: discord.Member, *, reason: str = None):
        ...
        if user.bot:
            await ctx.send(embed=embeds.error("Cannot Warn Bot", "You cannot warn or moderate bots."))
            return
        ...
        threshold_result = await apply_threshold(self.bot, ctx.guild, user, warn_count, moderator=ctx.author)
    ...
    async def timeout(self, ctx, user: discord.Member, duration: str = "10m", *, reason: str = None):
        ...
        await ctx.send(embed=embeds.success("Timeout Applied",
                f"{user.mention} timed out for {duration}. Case #{case_id}"))
    ...
    async def untimeout(self, ctx, user: discord.Member, *, reason: str = None):
        ...
        await ctx.send(embed=embeds.success("Timeout Removed",
                f"{user.mention}'s timeout removed. Case #{case_id}"))
    ...
    async def unban(self, ctx, user_id: int, *, reason: str = None):
        ...
        try:
            ...
            await ctx.send(embed=embeds.error("Not Found",
                    "That user is not banned or doesn't exist.")); return
        except discord.Forbidden:
    ...
    async def massban(self, ctx: commands.Context) -> None:
        """Ban up to 1000 members at once. Owner only."""
        ...
        e = discord.Embed(title="🔨 Massban Complete",
    ...
    async def unlock(self, ctx, channel: discord.TextChannel = None, *, reason: str = None):
        ...
        await target.send(embed=embeds.success("Channel Unlocked",
                "This channel has been unlocked."))
        await ctx.send(embed=embeds.success("Channel Unlocked",
                f"{target.mention} is now unlocked."))
    ...
    @commands.guild_only()
    async def role_add(self, ctx, user: discord.Member, role: discord.Role, *, reason: str = None):
        ...
        await ctx.send(embed=embeds.success("Role Added",
                f"{role.mention} added to {user.mention}."))
    ...
    async def role_remove(self, ctx, user: discord.Member, role: discord.Role, *, reason: str = None):
        ...
        await ctx.send(embed=embeds.success("Role Removed",
                f"{role.mention} removed from {user.mention}."))
    ...
    async def giverole(self, ctx, role: discord.Role):
        ...
        if role >= bot_top:
            await ctx.send(f"❌ **{role.name}** is above or equal to my top role."); return
