...
class ModerationCog(commands.Cog, name="Moderation"):
    ...
    async def warn(
        ...
        # Count active warns and check threshold
        all_cases, _ = await db.get_cases(user.id, interaction.guild.id, page=1, page_size=500)
        warn_count   = sum(1 for c in all_cases if c.get("action") == "WARN" and c.get("active"))

        from cogs.warn_threshold import apply_threshold
        threshold_result = await apply_threshold(self.bot, interaction.guild, user, warn_count)

        desc = f"{user.mention} has been warned. Case #{case_id} — Total warns: **{warn_count}**"
        if threshold_result:
            desc += f"\n⚒️ Threshold triggered: {threshold_result}"

        await interaction.response.send_message(
            embed=embeds.success("Warning Issued", desc),
            ephemeral=True,
        )
