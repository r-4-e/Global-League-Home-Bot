...
class TicketControlView(discord.ui.View):
    ...
    async def claim_ticket(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            await interaction.channel.edit(topic=f"📌 Claimed by {member} ({member.id})")
        except discord.Forbidden:
            pass
        e = discord.Embed(
            title="🙋 Ticket Claimed",
            description=f"Claimed by {member.mention}. They'll assist you shortly.",
            color=0x3498DB,
        )
...
class TicketCog(commands.Cog, name="Tickets"):
    ...
    async def ticket_setup(
        ...
        *,
        ...
        if not ctx.author.guild_permissions.administrator and not _is_staff(ctx.author):
            await ctx.send(embed=_err("No Permission", "Only administrators can set up the ticket system."))
            return
    ...
    async def ticket_config(self, ctx):
        ...
        if not _is_staff(ctx.author):
            await ctx.send(embed=_err("No Permission", "Only staff can view ticket config."))
            return
