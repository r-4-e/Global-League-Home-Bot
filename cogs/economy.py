...
class EconomyCog(commands.Cog, name="Economy"):
    ...
    async def store(self, interaction: discord.Interaction) -> None:
        ...
        listings   = await _get_marketplace(interaction.guild.id)
        sym        = cfg.get("currency_symbol", DEFAULT_CURRENCY)
    ...
    async def buy(self, interaction: discord.Interaction, item_id: str) -> None:
        ...
        if item_id in items:
            ...
            await interaction.response.send_message(
                embed=_err("Not Found", f"Item `{item_id}` not found in store."), ephemeral=True
            )
            return

        # Can't buy your own listing
        if is_listing and str(item.get("seller_id")) == str(interaction.user.id):
            await interaction.response.send_message(
                embed=_err("Invalid", "You can't buy your own listing."), ephemeral=True
            )
            return
        ...
        if is_listing:
            ...
            if seller_id:
                seller_data = await _get_balance(interaction.guild.id, seller_id)
                await _set_balance(
                    interaction.guild.id, seller_id,
                    seller_data["wallet"] + price, seller_data["bank"]
                )
                await _add_audit(interaction.guild.id, seller_id, "marketplace_sale", price, item_id)

            # Remove listing after purchase
            del listings[item_id]
            await _save_marketplace(interaction.guild.id, listings)
                ...
                try:
                    ...
                    if seller:
                        notify_embed = discord.Embed(
                            ...
                            description=(
                                f"**{item['name']}** was purchased by {interaction.user.mention}!\n"
    ...
    @app_commands.command(name="store_list", description="List an item from your inventory for sale in the store.")
    ...
    async def store_list(
        ...
        if price <= 0:
            await interaction.response.send_message(
                embed=_err("Invalid Price", "Price must be greater than 0."), ephemeral=True
            )
            return

        # Get item info from admin store for name/emoji/description
        store_items = await _get_store(interaction.guild.id)
        ...
        listing_id = f"mkt_{interaction.user.id}_{int(_time.time())}"
    ...
    @app_commands.command(name="store_delist", description="Remove your listing from the store and get your item back.")
    @app_commands.describe(listing_id="Listing ID to remove (shown in /store)")
    @app_commands.guilds(discord.Object(id=GUILD_ID))
    async def store_delist(
        ...
        if listing_id not in listings:
            await interaction.response.send_message(
                embed=_err("Not Found", f"Listing `{listing_id}` not found."), ephemeral=True
            )
            return

        listing = listings[listing_id]

        if str(listing.get("seller_id")) != str(interaction.user.id):
            await interaction.response.send_message(
                embed=_err("Not Yours", "You can only remove your own listings."), ephemeral=True
            )
            return
...
class EconomyExtraCog(commands.Cog, name="EconomyExtra"):
    ...
    @app_commands.command(name="stock_prices", description="View current real-world stock prices.")
    @app_commands.guilds(discord.Object(id=GUILD_ID))
    async def stock_prices(self, interaction: discord.Interaction) -> None:
        await interaction.response.defer()
        cfg    = await _get_config(interaction.guild.id)
        sym    = cfg.get("currency_symbol", DEFAULT_CURRENCY)
        prices = await _get_live_prices(interaction.guild.id, self._http)
    ...
    @app_commands.command(name="stock_buy", description="Buy shares in a real-world stock.")
    @app_commands.describe(ticker="Ticker e.g. AAPL TSLA BTC-USD", shares="Number of shares to buy")
    @app_commands.guilds(discord.Object(id=GUILD_ID))
    async def stock_buy(self, interaction: discord.Interaction, ticker: str, shares: int) -> None:
        ...
        if ticker not in BASE_STOCKS:
            ...
            await interaction.response.send_message(
                embed=_err("Unknown Ticker", f"Available: {tickers}"), ephemeral=True
            )
            return
        if shares <= 0:
    ...
    @app_commands.describe(ticker="Ticker symbol", shares="Number of shares or 'all'")
    @app_commands.guilds(discord.Object(id=GUILD_ID))
    async def stock_sell(self, interaction: discord.Interaction, ticker: str, shares: str) -> None:
        ...
        if ticker not in BASE_STOCKS:
            await interaction.response.send_message(embed=_err("Unknown Ticker", f"`{ticker}` not found."), ephemeral=True)
            return
        ...
        if owned <= 0:
            await interaction.response.send_message(embed=_err("Not Owned", f"You don't own any {ticker} shares."), ephemeral=True)
            return
    ...
    @app_commands.command(name="market_crash", description="[Admin] Crash the entire stock market by a set percentage.")
    @app_commands.describe(
    ...
    async def market_crash(
        ...
        if not 1 <= percent <= 90:
            await interaction.response.send_message(
                embed=_err("Invalid", "Crash percentage must be between 1 and 90."), ephemeral=True
            )
            return
        ...
        old_prices = await _get_stock_prices(interaction.guild.id)

        # Apply crash to all stocks
        new_prices = await _crash_all_stocks(interaction.guild.id, percent)
        ...
        crash_embed.set_footer(text=f"Triggered by {interaction.user}  •  Use /stock_prices to see current prices")
        crash_embed.timestamp = datetime.now(timezone.utc)

        # Send announcement
        ch = announce_channel or interaction.channel
        try:
    ...
    @app_commands.command(name="market_recover", description="[Admin] Remove crash status and restore real prices.")
    @app_commands.guilds(discord.Object(id=GUILD_ID))
    async def market_recover(self, interaction: discord.Interaction) -> None:
        from utils.permissions import check_invoker_permission
        ok, msg = check_invoker_permission(interaction, "administrator")
        if not ok:
            await interaction.response.send_message(embed=_err("Permission Denied", msg), ephemeral=True)
        ...
        prices = await _get_stock_prices(interaction.guild.id)
        for ticker in prices:
            prices[ticker]["crashed"]    = False
            prices[ticker]["crash_mult"] = 1.0
            prices[ticker]["last_update"] = 0  # Force refresh on next fetch
        await db.upsert_automod_rule(STOCKS_RULE, True, prices, interaction.guild.id)
